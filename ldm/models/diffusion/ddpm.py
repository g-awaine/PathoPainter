...
class LatentDiffusion(DDPM):
    ...
    def get_input(
        ...
        mask_ = (1-mask)
        img = x
        masked_img = (img*mask_)

        if bs is not None:
            x = x[:bs]
        x = x.to(self.device)
        encoder_posterior = self.encode_first_stage(x)  # torch.Size([1, 3, 64, 64])
        z = self.get_first_stage_encoding(encoder_posterior).detach()  # torch.Size([1, 3, 64, 64])

        masked_img = masked_img.to(self.device)
        encoder_posterior_mask = self.encode_first_stage(masked_img)  # torch.Size([1, 3, 64, 64])
        z_masked = self.get_first_stage_encoding(encoder_posterior_mask).detach()  # torch.Size([1, 3, 64, 64])

        cc = torch.nn.functional.interpolate(mask, size=z_masked.shape[-2:])

        cond_mask = torch.cat((z_masked, cc), dim=1)
        ...
        if return_original_cond:
            out.append(xc)
            out.append(cond_mask)
        else:
            c['c_inpaint'] = cond_mask 
    ...
    def forward(self, x, c, *args, **kwargs):
        t = torch.randint(0, self.num_timesteps, (x.shape[0],), device=self.device).long()
        mask_cond = c['c_inpaint']  # torch.Size([1, 4, 64, 64])

        if self.model.conditioning_key is not None:
            assert c is not None
            if self.cond_stage_trainable:
                c = self.get_learned_conditioning(c)  # torch.Size([1, 1, 512])
            if self.shorten_cond_schedule:  # TODO: drop this option
                tc = self.cond_ids[t].to(self.device)
                c = self.q_sample(x_start=c, t=tc, noise=torch.randn_like(c.float()))
        return self.p_losses(x, c, t, mask_cond, *args, **kwargs)
    ...
    def p_losses(self, x_start, cond, t, mask_cond, noise=None):
        noise = default(noise, lambda: torch.randn_like(x_start))
        x_noisy = self.q_sample(x_start=x_start, t=t, noise=noise)
        x_noisy = torch.cat([x_noisy, mask_cond], dim=1)  # concatenate the mask to the noisy image
        model_output = self.apply_model(x_noisy, t, cond)  # torch.Size([1, 3, 64, 64])
    ...
    @torch.no_grad()
    def sample_log(self, cond, batch_size, ddim, ddim_steps, cond_mask=None, **kwargs):
        if ddim:
            ddim_sampler = DDIMSampler(self)
            shape = (self.channels, self.image_size, self.image_size)
            samples, intermediates = ddim_sampler.sample(ddim_steps, batch_size, shape, cond, verbose=False, cond_mask=cond_mask, **kwargs)
    ...
    def log_images(
        ...
        **kwargs,
        ...
        log = dict()
        z, c, x, xrec, xc, cond_mask = self.get_input(
            batch,
            self.first_stage_key,
            return_first_stage_outputs=True,
            force_c_encode=True,
